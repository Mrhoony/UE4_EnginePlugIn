...
void FButtonCommand::OnClick()
{
	...
	void* handle = mainFrame.GetParentWindow()->GetNativeWindow()->GetOSWindowHandle(); // handle == HWND
	IDesktopPlatform* platform = FDesktopPlatformModule::Get();

	TArray<FString> fileNames;
	platform->OpenFileDialog(handle, "Open Mesh to Binary", path, "", "Binary Files(*.bin)|*.bin", EFileDialogFlags::None, fileNames);
	...
	GLog->Logf(L"Vertex Count : %d", data.Indices.Num());
